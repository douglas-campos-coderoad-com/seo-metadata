...
- **Single dark theme, not light/dark pair.** The brief specifies one dark-first palette with no light-mode equivalents given, so all tokens live on `:root` directly rather than gated behind a `.dark` class; there is no theme toggle in this app to justify maintaining two palettes.
- **Severity system redefined to fit the palette**, replacing the earlier "stays independent, always green/amber/red" rule: `good` → Golden (`#FFCB6B`, the brief's literal "positive deltas" color), `critical`/`destructive` → a muted, desaturated tone derived from Sunrise's hue rather than a bright alarm red (per "calm neutral error states instead of alarm-red"), `warning` → a muted sand/amber between the two. `Sunrise` itself is reserved for `--primary` (the CTA/key-marker accent, used sparingly) and is not reused as a severity color, keeping "one accent, once per view" intact.
...
- Full light+dark palette pair — rejected as unrequested scope; the brief is single-theme and no toggle exists to use a second one.
