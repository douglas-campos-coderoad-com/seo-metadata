...
import type { AnalysisRun, AnalysisTarget, Finding } from '@/shared/types';
...
describe('computeSharedIssues', () => {
  it('surfaces a finding that recurs on 2+ targets in the project', () => {
    ...
    const shared = computeSharedIssues(1, targetIds, source);
  ...
  it('does not surface a finding present on only one target', () => {
    ...
    expect(computeSharedIssues(1, targetIds, source)).toHaveLength(0);
  });
